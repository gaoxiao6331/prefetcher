...
describe("Start Function", () => {
    ...
    test("should start server successfully", async () => {
        ...
        expect(mockFastify.listen).toHaveBeenCalledWith(expect.objectContaining({ port: 3000 }));
    });
    ...
    test("should handle unhandledRejection (non-debug)", async () => {
        ...
        if (rejectionHandler) {
            ...
            expect(mockFastify.alert).toHaveBeenCalledWith(expect.stringContaining("Unhandled Rejection"));
        }
    });
    ...
    test("should handle uncaughtException (non-debug)", async () => {
        ...
        if (exceptionHandler) {
            ...
            expect(mockFastify.alert).toHaveBeenCalledWith(expect.stringContaining("Uncaught Exception"));
        }
    });
