...
describe("JsDelivrService", () => {
	...
	beforeEach(async () => {
		...
		(axios.get as jest.Mock).mockResolvedValue({ data: { status: "finished" } });
		mockExecSuccess();
	...
	describe("update", () => {
		...
		test("should checkout remote branch if local branch missing", async () => {
			...
			expect(exec).toHaveBeenCalledWith(
				expect.stringContaining('git rev-parse --verify "origin/feature-branch"'),
				expect.any(Function)
			);
		});
	...
	describe("purgeJsDelivrCache", () => {
		test("should log success when purge returns finished", async () => {
			...
			await jsDelivrService['purgeJsDelivrCache']('ns', 'proj', 'file.js', 'branch');
		...
		test("should throw error when purge fails", async () => {
			...
			await expect(jsDelivrService['purgeJsDelivrCache']('ns', 'proj', 'file.js', 'branch'))
	...
	describe("verifyContentUpdate", () => {
		test("should return true when content matches", async () => {
			...
			const result = await jsDelivrService.verifyContentUpdate("url", "expected content");
		...
		test("should return false when content mismatches", async () => {
			...
			expect(fastifyMock.log.warn).toHaveBeenCalledWith(expect.anything(), expect.stringContaining("Content verification failed"));
		});

		test("should return false on network error", async () => {
			(axios.get as jest.Mock).mockRejectedValue(new Error("Network Error"));

			const result = await jsDelivrService.verifyContentUpdate("url", "content");
			expect(result).toBe(false);
			expect(fastifyMock.log.warn).toHaveBeenCalledWith(expect.stringContaining("Failed to verify"));
		});
	});
});
