...
describe("InterceptionBlankScreenService", () => {
    ...
    beforeEach(async () => {
        ...
        mockPage = {
            ...
            evaluate: jest.fn().mockImplementation(async (fn: any, ...args: any[]) => {
        ...
        service = (await InterceptionBlankScreenService.create(fastifyMock)) as InterceptionBlankScreenService;
    });

    describe("filter", () => {
        test("should retain critical resources and remove non-critical ones", async () => {
            const resources: CapturedResource[] = [
                { url: "critical.js", type: "script", status: 200, sizeKB: 10, requestTime: 0, responseTime: 0, durationMs: 0 },
                { url: "non-critical.js", type: "script", status: 200, sizeKB: 10, requestTime: 0, responseTime: 0, durationMs: 0 },
            ];
            const ctx: GenerateContext = { url: "http://test.com", capturedResources: resources };
        ...
        test("should handle request interception branches", async () => {
            const resources: CapturedResource[] = [
                { url: "test.js", type: "script", status: 200, sizeKB: 10, requestTime: 0, responseTime: 0, durationMs: 0 },
            ];
            const ctx: GenerateContext = { url: "http://test.com", capturedResources: resources };
            ...
            mockPage.goto.mockImplementation(async () => {
                ...
                const mockReqError = {
                    url: () => { throw new Error("Handler error"); },
                    isInterceptResolutionHandled: () => false
                };
            ...
            expect(fastifyMock.log.warn).toHaveBeenCalledWith(expect.stringContaining("[Interception] Request handling failed"));
        });

        test("should handle validation failure and retain resource (safety first)", async () => {
            const resources: CapturedResource[] = [
                { url: "fail.js", type: "script", status: 200, sizeKB: 10, requestTime: 0, responseTime: 0, durationMs: 0 },
            ];
            const ctx: GenerateContext = { url: "http://test.com", capturedResources: resources };
            ...
            expect(fastifyMock.log.error).toHaveBeenCalledWith(expect.any(Error), expect.stringContaining("Failed to validate resource"));
        });
    });
    ...
    describe("isBlankScreen", () => {
        test("should return true if screenshot fails", async () => {
            ...
            expect(fastifyMock.log.error).toHaveBeenCalledWith(expect.any(Error), "Screenshot-based blank screen detection failed");
        });
