    ...
    describe("Initialization", () => {
        ...
        test("should handle browser disconnected event", async () => {
            ...
            (puppeteer.launch as jest.Mock).mockImplementationOnce(async () => {
                ...
                browserWithListener.on.mockImplementation((event: string, listener: any) => {
        ...
        test("should handle browser init returning null", async () => {
            ...
            await expect(service.captureResources(TEST_URL)).rejects.toThrow("Failed to initialize browser");
        });
    ...
    describe("Response Processing", () => {
        ...
        test("should handle various HTTP status codes", async () => {
            ...
            mockPage.goto.mockImplementation(async () => {
                ...
                if (requestListener) {
                    await requestListener(createMockRequest({
                        headers: () => ({ "x-prefetcher-req-id": TEST_REQUEST_ID }),
                    }));
                }
                ...
                for (const status of statusCodes) {
                    if (responseListener) {
                        await responseListener(createMockResponse({
                            status: () => status,
            ...
            expect(fastifyMock.log.warn).not.toHaveBeenCalledWith(expect.stringContaining("Response processing failed"));
        });
        ...
        test("should handle buffer retrieval failure", async () => {
            ...
            expect(fastifyMock.log.warn).not.toHaveBeenCalledWith(expect.stringContaining("Buffer failed"));
        });
    });
